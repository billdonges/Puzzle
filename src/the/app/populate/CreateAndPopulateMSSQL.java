...
import the.app.populate.Database.Campaign;
import the.app.populate.Database.Realm;
...
public class CreateAndPopulateMSSQL 
{
	...
	public void insertTracking(Connection mssql, Database db) throws Exception
	{
		...
		String insert = "INSERT INTO TRACKING (event_type, subscriber_id, event_date, realm_id, campaign_id) VALUES (?,?,getDate(),?,?)";
	...
	public void clean(Connection mssql) throws Exception
	{
		...
		st.execute("DELETE FROM tracking WHERE id > 0");
		st.execute("DELETE FROM subscribers WHERE id > 0");
		st.close();
		st = null;
	}
}
